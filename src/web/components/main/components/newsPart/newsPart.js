...
const NewsPart = () => {
    ...
    let { data, isLoading } = useQuery(['requiredNews', ''], async () => {

        const res = await axios.get(baseUrl + `news/home${'/category/'}?include=homeNews,parent`);
                                        ...
                                        isLoading === false && (
                                            data.data.map(item => (
                                                <Tab eventKey={item.name} title={item.name} key={item.id}>
                                                    <div className='newsPart__content '>
                                                        {
                                                            item.homeNews.data.map((subitem) => (
                                                                <div className='newsPart__contentBox' data-different='Digər' key={subitem.id}>
                                                                        ...
                                                                        <div className='newsPart__in'>
                                                                            ...
                                                                            <div className='newsPart__info'>
                                                                                <div className='share'>
                                                                                    <span>
                                                                                        {
                                                                                            subitem.post_date.split(' ')[1].split(':')[0] + ':' + subitem.post_date.split(' ')[1].split(':')[1]
                                                                                        }
                                                                                    </span>
                                                                                    {
                                                    ...
                                                    <NavLink to={`/category/${item.parent.data.slug !== undefined ? item.parent.data.slug + '/' : ''}` + item.slug} className='newsPart__allNews'>
                                                        Daha çox
                                                        {

                                                        }
                                                        <svg width="8" height="12" viewBox="0 0 8 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M0.814453 9.95749L5.08067 5.68195L0.814453 1.40642L2.12785 0.0930176L7.71679 5.68195L2.12785 11.2709L0.814453 9.95749Z" fill="white" />
                                                        </svg>
                                                    </NavLink>
                                                </Tab>
                                            ))
