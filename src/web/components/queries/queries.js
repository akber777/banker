...
export const vacancySelect = async (key, data) => {
  const res = await axios.get(
    baseUrl +
      `vacancy/selectable?include=${vacancySelectable}${key.queryKey[1]}`
  );
...
export const homeCategory = async (key, data) => {
  const res = await axios.get(
    baseUrl + `news/menu/homepage-category${key.queryKey[1]}`
  );
